...
-- =============================================================================
--
-- Entry point: FreeSWITCH dialplan for ERS trigger numbers (e.g. 1222).
--
-- Flow:
--   1. GET /internal/ers/lookup?number=<dest>
--      Full config: slots, responders, queue settings, gateway, recording
--   2. Determine slot (primary / secondary / queued) from API response
--   3. POST /internal/ers/incidents — create ACTIVE or QUEUED incident
--   4. QUEUED path:
--      a. Play queue announcement + hold music
--      b. Poll GET /internal/ers/incidents/<uuid>/status every POLL_MS ms
--      c. Break when status → ACTIVE; invite primary responders
--   5. ACTIVE path:
--      a. Start recording if cfg.record_conferences
--      b. Invite tier responders via freeswitch.bgapi() — non-blocking
--      c. session:execute("conference", …) — blocks until caller disconnects
--   6. POST /internal/ers/incidents/<uuid>/complete — close incident + recording path
--
-- Dialplan example:
