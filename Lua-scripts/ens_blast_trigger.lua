...
-- =============================================================================
--
-- Entry point: FreeSWITCH dialplan for ENS blast trigger numbers (e.g. 1888).
--
-- Flow:
--   1. GET /internal/ens/lookup?number=<dest> — fetch ENS config
--   2. If cfg.pin_required → collect DTMF → POST /internal/ens/verify-pin
--      (max 3 attempts; hangup CALL_REJECTED on failure)
--   3. Play prompt → record blast message to ENRS_REC_DIR/<filename>.wav
--      (validates file ≥ 5000 bytes; hangup if too short)
--   4. POST /internal/ens/campaign/start — backend manages outbound delivery
--   5. Speak confirmation (N contacts notified) and hangup
--
-- Dialplan example:
...
--   ENRS_REC_DIR       — ENS recording directory (default: /opt/freeswitch/recordings/ens)
...
local function speak(text)
  ...
  if PIPER_URL == "" then
    freeswitch.consoleLog("WARN", "[ENS_BLAST] PIPER_LUA_URL not set — prompt is silent\n")
    return
  end
  ...
  freeswitch.consoleLog("ERR",
    "[ENS_BLAST] Piper synthesis failed (url=" .. PIPER_URL .. " fsize=" .. fsize .. ") — prompt is silent\n")
  os.remove(wav_path)
